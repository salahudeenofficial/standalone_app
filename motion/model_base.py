...
def model_sampling(model_config, model_type):
    s = motion.model_sampling.ModelSamplingDiscrete

    if model_type == ModelType.EPS:
        c = motion.model_sampling.EPS
    elif model_type == ModelType.V_PREDICTION:
        c = motion.model_sampling.V_PREDICTION
    elif model_type == ModelType.V_PREDICTION_EDM:
        c = motion.model_sampling.V_PREDICTION
        s = motion.model_sampling.ModelSamplingContinuousEDM
    elif model_type == ModelType.FLOW:
        c = motion.model_sampling.CONST
        s = motion.model_sampling.ModelSamplingDiscreteFlow
    elif model_type == ModelType.STABLE_CASCADE:
        c = motion.model_sampling.EPS
        s = motion.model_sampling.StableCascadeSampling
    elif model_type == ModelType.EDM:
        c = motion.model_sampling.EDM
        s = motion.model_sampling.ModelSamplingContinuousEDM
    elif model_type == ModelType.V_PREDICTION_CONTINUOUS:
        c = motion.model_sampling.V_PREDICTION
        s = motion.model_sampling.ModelSamplingContinuousV
    elif model_type == ModelType.FLUX:
        c = motion.model_sampling.CONST
        s = motion.model_sampling.ModelSamplingFlux
    elif model_type == ModelType.IMG_TO_IMG:
        c = motion.model_sampling.IMG_TO_IMG
    elif model_type == ModelType.FLOW_COSMOS:
        c = motion.model_sampling.COSMOS_RFLOW
        s = motion.model_sampling.ModelSamplingCosmosRFlow
...
class BaseModel(torch.nn.Module):
    def __init__(self, model_config, model_type=ModelType.EPS, device=None, unet_model=None):
        ...
        if not unet_config.get("disable_unet_model_creation", False):
            if model_config.custom_operations is None:
                ...
                operations = motion.ops.pick_operations(unet_config.get("dtype", None), self.manual_cast_dtype, fp8_optimizations=fp8, scaled_fp8=model_config.scaled_fp8)
            else:
    ...
    def apply_model(self, x, t, c_concat=None, c_crossattn=None, control=None, transformer_options={}, **kwargs):
        return motion.patcher_extension.WrapperExecutor.new_class_executor(
            self._apply_model,
            self,
            motion.patcher_extension.get_all_wrappers(motion.patcher_extension.WrappersMP.APPLY_MODEL, transformer_options)
    ...
    def extra_conds(self, **kwargs):
        ...
        if concat_cond is not None:
            out['c_concat'] = motion.conds.CONDNoiseShape(concat_cond)

        adm = self.encode_adm(**kwargs)
        if adm is not None:
            out['y'] = motion.conds.CONDRegular(adm)

        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out['c_crossattn'] = motion.conds.CONDCrossAttn(cross_attn)

        cross_attn_cnet = kwargs.get("cross_attn_controlnet", None)
        if cross_attn_cnet is not None:
            out['crossattn_controlnet'] = motion.conds.CONDCrossAttn(cross_attn_cnet)

        c_concat = kwargs.get("noise_concat", None)
        if c_concat is not None:
            out['c_concat'] = motion.conds.CONDNoiseShape(c_concat)
...
class WAN21(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, image_to_video=False, device=None):
        super().__init__(model_config, model_type, device=device, unet_model=motion.ldm.wan.model.WanModel)
    ...
    def extra_conds(self, **kwargs):
        ...
        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out['c_crossattn'] = motion.conds.CONDRegular(cross_attn)

        clip_vision_output = kwargs.get("clip_vision_output", None)
        if clip_vision_output is not None:
            out['clip_fea'] = motion.conds.CONDRegular(clip_vision_output.penultimate_hidden_states)

        time_dim_concat = kwargs.get("time_dim_concat", None)
        if time_dim_concat is not None:
            out['time_dim_concat'] = motion.conds.CONDRegular(self.process_latent_in(time_dim_concat))

        reference_latents = kwargs.get("reference_latents", None)
        if reference_latents is not None:
            out['reference_latent'] = motion.conds.CONDRegular(self.process_latent_in(reference_latents[-1])[:, :, 0])
...
class WAN21_Vace(WAN21):
    def __init__(self, model_config, model_type=ModelType.FLOW, image_to_video=False, device=None):
        super(WAN21, self).__init__(model_config, model_type, device=device, unet_model=motion.ldm.wan.model.VaceWanModel)
        self.image_to_video = image_to_video
    ...
    def extra_conds(self, **kwargs):
        ...
        out['vace_context'] = motion.conds.CONDRegular(vace_frames)

        vace_strength = kwargs.get("vace_strength", [1.0] * len(vace_frames_out))
        out['vace_strength'] = motion.conds.CONDConstant(vace_strength)
        return out

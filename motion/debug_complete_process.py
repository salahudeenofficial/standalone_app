#!/usr/bin/env python3
"""
Debug the complete LoRA loading process
"""
...
def debug_complete_process():
    """Debug the complete LoRA loading process"""
    print("🔍 DEBUGGING COMPLETE LORA PROCESS")
    print("="*60)
    
    # Create a model that matches your WAN structure
    model_sd = {
    ...
    print(f"\n🔧 STEP 1: TESTING CONVERSION")
    converted_lora = convert_lora(lora_sd)
    print(f"After conversion:")
    for key in converted_lora.keys():
        print(f"  {key}")
    
    # Step 2: Test key mapping
    print(f"\n🔧 STEP 2: TESTING KEY MAPPING")
    ...
    # Step 3: Test load_lora function
    print(f"\n🔧 STEP 3: TESTING LOAD_LORA FUNCTION")
    ...
    # Step 4: Test add_patches
    print(f"\n🔧 STEP 4: TESTING ADD_PATCHES")
    ...
    # Step 5: Test complete process
    print(f"\n🔧 STEP 5: TESTING COMPLETE PROCESS")
    new_unet2, new_clip2 = load_lora_for_models(unet_patcher, None, lora_sd)
    
    if new_unet2 is not None:
        patches_count = len(new_unet2.patches)
        print(f"✅ Patches applied: {patches_count}")
        
        if patches_count > 0:
            print("🎉 SUCCESS: Complete process working!")
            print("Patches:")
            for key, patches in new_unet2.patches.items():
                print(f"  {key}: {len(patches)} patch(es)")
        else:
            print("❌ FAILED: Complete process not working")

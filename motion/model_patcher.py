...
import motion.utils
from motion.comfy_types import UnetWrapperFunction
from motion.patcher_extension import PatcherInjection,CallbacksMP,WrappersMP
...
def create_model_options_clone(orig_model_options: dict):
    return motion.patcher_extension.copy_nested_dicts(orig_model_options)
...
class LowVramPatch:
    ...
    def __call__(self, weight):
        ...
        if intermediate_dtype not in [torch.float32, torch.float16, torch.bfloat16]: #intermediate_dtype has to be one that is supported in math ops
            ...
            return motion.float.stochastic_rounding(motion.lora.calculate_weight(self.patches[self.key], weight.to(intermediate_dtype), self.key, intermediate_dtype=intermediate_dtype), weight.dtype, seed=string_to_seed(self.key))

        return motion.lora.calculate_weight(self.patches[self.key], weight, self.key, intermediate_dtype=intermediate_dtype)
...
def get_key_weight(model, key):
    ...
    if len(op_keys) < 2:
        weight = motion.utils.get_attr(model, key)
    else:
        op = motion.utils.get_attr(model, op_keys[0])
        try:
...
class ModelPatcher:
    def __init__(self, model, load_device, offload_device, size=0, weight_inplace_update=False):
        ...
        self.hook_patches: dict[motion.hooks._HookRef] = {}
        self.hook_patches_backup: dict[motion.hooks._HookRef] = None
        self.hook_backup: dict[str, tuple[torch.Tensor, torch.device]] = {}
        self.cached_hook_patches: dict[motion.hooks.HookGroup, dict[str, torch.Tensor]] = {}
        self.current_hooks: Optional[motion.hooks.HookGroup] = None
        self.forced_hooks: Optional[motion.hooks.HookGroup] = None  # NOTE: only used for CLIP at this time
        self.is_clip = False
        self.hook_mode = motion.hooks.EnumHookMode.MaxSpeed
    ...
    def model_size(self):
        if self.size > 0:
            return self.size
        self.size = motion.model_management_standalone.module_size(self.model)
        return self.size
    ...
    def get_model_object(self, name: str) -> torch.nn.Module:
        ...
        if name in self.object_patches:
            ...
            if name in self.object_patches_backup:
                ...
                return motion.utils.get_attr(self.model, name)
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        if device_to is not None:
            temp_weight = motion.model_management_standalone.cast_to_device(weight, device_to, torch.float32, copy=True)
        else:
            temp_weight = weight.to(torch.float32, copy=True)
        if convert_func is not None:
            temp_weight = convert_func(temp_weight, inplace=True)

        out_weight = motion.lora.calculate_weight(self.patches[key], temp_weight, key)
        if set_func is None:
            out_weight = motion.float.stochastic_rounding(out_weight, weight.dtype, seed=string_to_seed(key))
            if inplace_update:
                motion.utils.copy_to_param(self.model, key, out_weight)
            else:
                motion.utils.set_attr_param(self.model, key, out_weight)
        else:
    ...
    def _load_list(self):
        ...
        for n, m in self.model.named_modules():
            ...
            if not skip and (hasattr(m, "comfy_cast_weights") or len(params) > 0):
                loading.append((motion.model_management_standalone.module_size(m), n, m, params))
        return loading
    ...
    def patch_model(self, device_to=None, lowvram_model_memory=0, load_weights=True, force_patch_weights=False):
        with self.use_ejected():
            for k in self.object_patches:
                old = motion.utils.set_attr(self.model, k, self.object_patches[k])
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        if unpatch_weights:
            ...
            for k in keys:
                ...
                if bk.inplace_update:
                    motion.utils.copy_to_param(self.model, k, bk.weight)
                else:
                    motion.utils.set_attr_param(self.model, k, bk.weight)
        ...
        for k in keys:
            motion.utils.set_attr(self.model, k, self.object_patches_backup[k])
                ...
                if hasattr(m, "comfy_patched_weights") and m.comfy_patched_weights == True:
                    ...
                    for param in params:
                        ...
                        if bk is not None:
                            ...
                            if bk.inplace_update:
                                motion.utils.copy_to_param(self.model, key, bk.weight)
                            else:
                                motion.utils.set_attr_param(self.model, key, bk.weight)
                            self.backup.pop(key)
    ...
    def calculate_weight(self, patches, weight, key, intermediate_dtype=torch.float32):
        logging.warning("The ModelPatcher.calculate_weight function is deprecated, please use: motion.lora.calculate_weight instead")
        return motion.lora.calculate_weight(patches, weight, key, intermediate_dtype=intermediate_dtype)
    ...
    def set_hook_mode(self, hook_mode: motion.hooks.EnumHookMode):
        self.hook_mode = hook_mode

    def prepare_hook_patches_current_keyframe(self, t: torch.Tensor, hook_group: motion.hooks.HookGroup, model_options: dict[str]):
        curr_t = t[0]
    ...
    def register_all_hook_patches(self, hooks: motion.hooks.HookGroup, target_dict: dict[str], model_options: dict=None,
                                  registered: motion.hooks.HookGroup = None):
        self.restore_hook_patches()
        if registered is None:
            registered = motion.hooks.HookGroup()
        # handle WeightHooks
        weight_hooks_to_register: list[motion.hooks.WeightHook] = []
        for hook in hooks.get_type(motion.hooks.EnumHookType.Weight):
    ...
    def add_hook_patches(self, hook: motion.hooks.WeightHook, patches, strength_patch=1.0, strength_model=1.0):
        with self.use_ejected():
    ...
    def get_combined_hook_patches(self, hooks: motion.hooks.HookGroup):
    ...
    def apply_hooks(self, hooks: motion.hooks.HookGroup, transformer_options: dict=None, force_apply=False):
        ...
        if self.current_hooks == hooks and (not force_apply or (not self.is_clip and hooks is None)):
            return motion.hooks.create_transformer_options_from_hooks(self, hooks, transformer_options)
        self.patch_hooks(hooks=hooks)
        for callback in self.get_all_callbacks(CallbacksMP.ON_APPLY_HOOKS):
            callback(self, hooks)
        return motion.hooks.create_transformer_options_from_hooks(self, hooks, transformer_options)

    def patch_hooks(self, hooks: motion.hooks.HookGroup):
        with self.use_ejected():
            if hooks is not None:
                model_sd_keys = list(self.model_state_dict().keys())
                memory_counter = None
                if self.hook_mode == motion.hooks.EnumHookMode.MaxSpeed:
                    # TODO: minimum_counter should have a minimum that conforms to loaded model requirements
                    memory_counter = MemoryCounter(initial=motion.model_management_standalone.get_free_memory(self.load_device),
                                                minimum=motion.model_management_standalone.minimum_inference_memory()*2)
    ...
    def patch_cached_hook_weights(self, cached_weights: dict, key: str, memory_counter: MemoryCounter):
        if key not in self.hook_backup:
            weight: torch.Tensor = motion.utils.get_attr(self.model, key)
        ...
        motion.utils.copy_to_param(self.model, key, cached_weights[key][0].to(device=cached_weights[key][1]))

    def clear_cached_hook_weights(self):
        self.cached_hook_patches.clear()
        self.patch_hooks(None)

    def patch_hook_weight_to_device(self, hooks: motion.hooks.HookGroup, combined_patches: dict, key: str, original_weights: dict, memory_counter: MemoryCounter):
        ...
        temp_weight = motion.model_management_standalone.cast_to_device(weight, weight.device, torch.float32, copy=True)
        if convert_func is not None:
            temp_weight = convert_func(temp_weight, inplace=True)

        out_weight = motion.lora.calculate_weight(combined_patches[key],
                                                 temp_weight,
            ...
            if whitelist_keys_set:
                for k in keys:
                    if k in whitelist_keys_set:
                        motion.utils.copy_to_param(self.model, k, self.hook_backup[k][0].to(device=self.hook_backup[k][1]))

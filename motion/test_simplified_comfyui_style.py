#!/usr/bin/env python3
"""
Simplified test for ComfyUI-style partial loading
Tests the core functionality without requiring real model files
"""
...
from comfyui_style_model_loader import ComfyUIStyleModelPatcher
...
def test_device_handling():
    """Test device handling in ComfyUI operations"""
    print("🧪 TESTING DEVICE HANDLING IN COMFYUI OPERATIONS")
    print("=" * 60)
    
    # Test ComfyUILinear
    print("🔧 Testing ComfyUILinear...")
    linear = ComfyUILinear(100, 50)
    linear.to('cpu')  # Move to CPU
    
    # Create input on GPU
    if torch.cuda.is_available():
        input_tensor = torch.randn(1, 100).to('cuda')
        
        try:
            with torch.no_grad():
                output = linear(input_tensor)
            print(f"   ✅ ComfyUILinear device handling: Success")
            print(f"   Input device: {input_tensor.device}")
            print(f"   Output device: {output.device}")
            print(f"   Weight device: {linear.weight.device}")
        except Exception as e:
            print(f"   ❌ ComfyUILinear device handling failed: {e}")
            return False
    
    # Test ComfyUIConv2d
    print("🔧 Testing ComfyUIConv2d...")
    conv = ComfyUIConv2d(3, 64, 3, padding=1)
    conv.to('cpu')  # Move to CPU
    ...
    if torch.cuda.is_available():
        ...
        try:
            ...
            print(f"   ✅ ComfyUIConv2d device handling: Success")
            print(f"   Input device: {input_tensor.device}")
            print(f"   Output device: {output.device}")
            print(f"   Weight device: {conv.weight.device}")
        except Exception as e:
            print(f"   ❌ ComfyUIConv2d device handling failed: {e}")
            return False
...
def test_comfyui_style_model_patcher():
    ...
    print("\n🧪 TESTING COMFYUI-STYLE MODEL PATCHER")
    print("=" * 60)
...
def main():
    """Main test function"""
    print("🚀 SIMPLIFIED COMFYUI-STYLE PARTIAL LOADING TEST")
    print("=" * 80)
    
    # Run all tests
    tests = [
        ("Device Handling in ComfyUI Operations", test_device_handling),
    ...
    if passed_tests == total_tests:
        print(f"\n🎉 ALL TESTS PASSED!")
        print(f"   ComfyUI-style partial loading is working!")
        print(f"   Device handling: ✅")

...
import motion.weight_adapter as weight_adapter
import logging
import torch
...
def model_lora_keys_unet(model, key_map={}):
    ...
    diffusers_keys = motion.utils.unet_to_diffusers(model.model_config.unet_config)
    ...
    if isinstance(model, motion.model_base.StableCascade_C):
        for k in sdk:
            if k.startswith("diffusion_model."):
                if k.endswith(".weight"):
                    key_lora = k[len("diffusion_model."):-len(".weight")].replace(".", "_")
    ...
    if isinstance(model, motion.model_base.SD3): #Diffusers lora SD3
        diffusers_keys = motion.utils.mmdit_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, motion.model_base.AuraFlow): #Diffusers lora AuraFlow
        diffusers_keys = motion.utils.auraflow_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, motion.model_base.PixArt):
        diffusers_keys = motion.utils.pixart_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, motion.model_base.HunyuanDiT):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
                key_lora = k[len("diffusion_model."):-len(".weight")]
                key_map["base_model.model.{}".format(key_lora)] = k #official hunyuan lora format

    if isinstance(model, motion.model_base.Flux): #Diffusers lora Flux
        diffusers_keys = motion.utils.flux_to_diffusers(model.model_config.unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, motion.model_base.GenmoMochi):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"): #Official Mochi lora format
                key_lora = k[len("diffusion_model."):-len(".weight")]
                key_map["{}".format(key_lora)] = k

    if isinstance(model, motion.model_base.HunyuanVideo):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
    ...
    if isinstance(model, motion.model_base.HiDream):
        for k in sdk:
            if k.startswith("diffusion_model."):
                if k.endswith(".weight"):
                    key_lora = k[len("diffusion_model."):-len(".weight")]
    ...
    if isinstance(model, motion.model_base.ACEStep):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"): #Official ACE step lora format
                key_lora = k[len("diffusion_model."):-len(".weight")]
                key_map["{}".format(key_lora)] = k

    if isinstance(model, motion.model_base.QwenImage):
        for k in sdk:
...
def calculate_weight(patches, weight, key, intermediate_dtype=torch.float32, original_weights=None):
    for p in patches:
        ...
        if isinstance(v, list):
            v = (calculate_weight(v[1:], v[0][1](motion.model_management_standalone.cast_to_device(v[0][0], weight.device, intermediate_dtype, copy=True), inplace=True), key, intermediate_dtype=intermediate_dtype), )
        ...
        if patch_type == "diff":
            ...
            if strength != 0.0:
                if diff.shape != weight.shape:
                    ...
                    weight += function(strength * motion.model_management_standalone.cast_to_device(diff, weight.device, weight.dtype))
        elif patch_type == "set":
            weight.copy_(v[0])
        elif patch_type == "model_as_lora":
            target_weight: torch.Tensor = v[0]
            diff_weight = motion.model_management_standalone.cast_to_device(target_weight, weight.device, intermediate_dtype) - \
                          motion.model_management_standalone.cast_to_device(original_weights[key][0][0], weight.device, intermediate_dtype)
            weight += function(strength * motion.model_management_standalone.cast_to_device(diff_weight, weight.device, weight.dtype))
        else:

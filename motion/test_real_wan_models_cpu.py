#!/usr/bin/env python3
"""
Test script for the new real WAN model loading - CPU only version
"""
...
def test_model_creation_cpu():
    """Test creating a real WAN model on CPU"""
    print("\n🔧 Testing Model Creation (CPU)...")
    
    try:
        from model_detection import create_model_from_config
        
        # Test config for VaceWanModel (smaller for CPU)
        model_config = {
            "image_model": "wan2.1",
            "model_type": "vace",
            "patch_size": (1, 2, 2),
            "text_len": 512,
            "in_dim": 16,
            "dim": 512,  # Smaller dimension for CPU
            "ffn_dim": 2048,  # Smaller FFN for CPU
            "freq_dim": 256,
            "text_dim": 4096,
            "out_dim": 16,
        ...
        model = create_model_from_config(model_config, device=device, dtype=torch.float32)
        print(f"   ✅ Created model: {type(model).__name__}")
        
        # Test forward pass with smaller dummy inputs
        x = torch.randn(1, 16, 4, 32, 32, device=device)  # Smaller dimensions
...
def test_pipeline_integration():
    ...
    try:
        # Test that our new model loading works with the pipeline
        from pipeline import WanVideoPipeline
        
        # Create a minimal pipeline instance
        pipeline = WanVideoPipeline()
        
        # Test that the pipeline can use our new model loading
        print(f"   ✅ Pipeline created: {type(pipeline).__name__}")
        print(f"   ✅ Pipeline has step_2_unet_clip_lora method: {hasattr(pipeline, 'step_2_unet_clip_lora')}")
...
def main():
    """Run all tests"""
    print("🚀 Testing Real WAN Model Implementation (CPU Version)")
    print("="*60)
    
    tests = [
        ("Model Detection", test_model_detection),
        ("Model Creation (CPU)", test_model_creation_cpu),
    ...
    if all_passed:
        ...
        print("✅ The UNet loading issue has been fixed!")
    else:

...
Comprehensive test script for Step 2 model loading and verification
"""
...
def test_step2_unet_clip_loading():
    ...
    try:
        ...
        if missing_models:
            ...
            return test_model_class_initialization()
        
        # Test UNet loading
        print(f"\n🧠 Testing UNet loading...")
        unet_results = test_unet_loading(unet_model_path)
        
        # Test CLIP loading
        print(f"\n📝 Testing CLIP loading...")
        clip_results = test_clip_loading(clip_model_path)
        
        # Test combined loading (Step 2 style)
        print(f"\n🔗 Testing combined UNet + CLIP loading (Step 2 style)...")
        combined_results = test_combined_loading(unet_model_path, clip_model_path)
...
def test_unet_loading(unet_model_path):
...
def test_clip_loading(clip_model_path):
...
def test_combined_loading(unet_model_path, clip_model_path):
...
def test_model_class_initialization():
...
def main():
    """Main test function"""
    print("🚀 Step 2: UNet + CLIP Loading Test")
    print("="*80)

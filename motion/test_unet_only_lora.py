#!/usr/bin/env python3
"""
Test the fix for UNet-only LoRA application
"""

import torch
from lora import load_lora_for_models, model_lora_keys_unet, model_lora_keys_clip
from standalone_sd import WANModel, T5CLIPModel
from standalone_model_patcher import create_model_patcher

def test_unet_only_lora():
    """Test that LoRA is applied only to UNet, not CLIP"""
    print("🧪 TESTING UNET-ONLY LORA APPLICATION")
    print("="*60)
    ...
    clip_sd = {
        ...
        "t5xxl.transformer.encoder.block.0.layer.0.SelfAttention.k.weight": torch.randn(4096, 4096),
    }
    ...
    lora_sd = {
        ...
        "diffusion_model.blocks.0.self_attn.k.lora_down.weight": torch.randn(32, 1024),
        "diffusion_model.blocks.0.self_attn.k.lora_up.weight": torch.randn(1024, 32),
        "diffusion_model.blocks.0.self_attn.k.alpha": torch.tensor(32.0),
    }
    ...
    print(f"UNet key mappings: {len(unet_key_map)}")
    print(f"CLIP key mappings: {len(clip_key_map)}")
    
    # Show some UNet mappings
    print("UNet mappings (first 5):")
    for i, (lora_key, model_key) in enumerate(list(unet_key_map.items())[:5]):
        print(f"  {lora_key} -> {model_key}")
    
    # Show some CLIP mappings
    print("CLIP mappings (first 5):")
    for i, (lora_key, model_key) in enumerate(list(clip_key_map.items())[:5]):
        print(f"  {lora_key} -> {model_key}")

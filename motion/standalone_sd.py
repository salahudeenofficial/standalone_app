...
class CLIP:
    def __init__(self, target=None, embedding_directory=None, no_init=False, tokenizer_data={}, parameters=0, model_options={}):
        ...
        self.patcher = motion.model_patcher.ModelPatcher(self.cond_stage_model, load_device=load_device, offload_device=offload_device)
        self.patcher.hook_mode = motion.hooks.EnumHookMode.MinVram
...
def load_clip(ckpt_paths, clip_type=CLIPType.STABLE_DIFFUSION, model_options={}):
    print(clip_type)
    """Load CLIP model from checkpoint paths"""
    clip_data = []
    for p in ckpt_paths:
        clip_data.append(motion.utils.load_torch_file(p, safe_load=True))
    # embedding_directory =  os.path.join(os.path.dirname(os.path.abspath(__file__)), "embeddings")
    return load_text_encoder_state_dicts(clip_data, embedding_directory=None, clip_type=clip_type, model_options=model_options)
...
def load_diffusion_model_state_dict(sd, model_options={}):
    ...
    if model_config is not None:
        ...
        if new_sd is not None: #diffusers mmdit
            ...
            diffusers_keys = motion.utils.unet_to_diffusers(model_config.unet_config)
    ...
    return motion.model_patcher.ModelPatcher(model, load_device=load_device, offload_device=offload_device)
...
def load_state_dict_guess_config(sd, output_vae=True, output_clip=True, output_clipvision=False, embedding_directory=None, output_model=True, model_options={}, te_model_options={}, metadata=None):
    ...
    diffusion_model_prefix = motion.model_detection.unet_prefix_from_state_dict(sd)
    parameters = motion.utils.calculate_parameters(sd, diffusion_model_prefix)
    weight_dtype = motion.utils.weight_dtype(sd, diffusion_model_prefix)
    load_device = model_management.get_torch_device()

    model_config = motion.model_detection.model_config_from_unet(sd, diffusion_model_prefix, metadata=metadata)
    if model_config is None:
        logging.warning("Warning, This is not a checkpoint file, trying to load it as a diffusion model only.")
    ...
    if output_vae:
        vae_sd = motion.utils.state_dict_prefix_replace(sd, {k: "" for k in model_config.vae_key_prefix}, filter_keys=True)
    ...
    if output_clip:
        ...
        if clip_target is not None:
            ...
            if len(clip_sd) > 0:
                parameters = motion.utils.calculate_parameters(clip_sd)
    ...
    if output_model:
        model_patcher = motion.model_patcher.ModelPatcher(model, load_device=load_device, offload_device=model_management.unet_offload_device())

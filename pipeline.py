...
class ReferenceVideoPipeline:
    ...
    def _comfy_vae_encode(self, vae, pixel_samples):
        """Encode pixel samples using ComfyUI's VAE with focused debugging"""
        # Create a focused debugger wrapper around the VAE
        debugger = FocusedVAEDebugger(vae)
        
        # Encode with focused debugging
        ...
        try:
            ...
            if ref_img is not None:
                ...
                try:
                    ...
                    # self.investigate_unaccounted_memory()  # COMMENTED OUT - old debugging
...
class FocusedVAEDebugger:
    ...
    def encode(self, pixel_samples, call_type="Unknown"):
        ...
        print(f"\n🔍 DEBUG: VAE.encode() call #{self.call_counter} ({call_type})")
        print("=" * 80)
        
        # 1. GPU Memory Status BEFORE encode
        print("📊 GPU MEMORY STATUS BEFORE .encode():")
        ...
        print(f"   Memory: {pixel_samples.numel() * pixel_samples.element_size() / 1024**2:.2f} MB")
        print(f"   Contiguous: {pixel_samples.is_contiguous()}")
        print(f"   Requires grad: {pixel_samples.requires_grad}")
        ...
        print("\n💾 CUDA MEMORY SUMMARY:")
        try:
            memory_summary = torch.cuda.memory_summary(device=0, abbreviated=False)
            # Extract key lines
            for line in memory_summary.split('\n'):
                if any(keyword in line for keyword in ['Allocated memory', 'Reserved memory', 'Active memory', 'Inactive memory']):
                    print(f"   {line.strip()}")
        except Exception as e:
        ...
        try:
            ...
            print(f"✅ VAE.encode() call #{self.call_counter} SUCCESS")
            return result
        except Exception as e:
            print(f"❌ VAE.encode() call #{self.call_counter} FAILED: {type(e).__name__}: {e}")
            raise
